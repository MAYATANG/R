...
df <- readr::read_csv("./raw/winemag-data_first150k.csv")
df

# 2. 請問 country 欄位，在排除 NA 值後，共有幾個unduplicate unit特殊值?
t <- df %>% drop_na(country)
...
df <- df %>% mutate(NewPrice = price*30)

# 5. 請創造一新欄位 IsFrance
# - 當 country==France 的時候，該值為 T。
# - 當 country!=France 的時候，該值為 F。
IsFrance <-  function(country){
  if(country == France){
    return("T")
  }
   return("F") 
